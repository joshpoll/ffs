...
function VizTrace(Props) {
  ...
  var padding = paddingOpt !== undefined ? paddingOpt : 10;
  var transition = transitionOpt !== undefined ? transitionOpt : false;
  ...
  var match$1 = List.split(trace);
  var match$2 = List.split(match$1[0]);
  var initState;
  if (transition) {
    var nextPos = Caml_primitive.caml_int_min(state.pos + 1 | 0, state.length - 1 | 0);
    initState = Main$Sidewinder.renderTransition(false, undefined, state.prevState, state.currState, List.nth(swTrace, state.pos), List.nth(match$2[1], state.pos), List.nth(swTrace, nextPos));
  } else {
