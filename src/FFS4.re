...
let step = (c: config): option(config) =>
  ...
  | {focus: Value(v), frame: {ctxts: [], env}, stack: [frame, ...stack]} =>
    Some({focus: Value(v), frame, stack})
  /* let */
  /* let_begin */
  /*
   ...
   */
  | {focus: AExp(Num(n)), frame, stack} => Some({focus: Value(VNum(n)), frame, stack})
  /* add */
  /* add_begin */
  /*
  ...
  /*
    ...
    small_step ⟨focus.exp (exp.lift a), f, fs⟩
               ⟨focus.aexp a, f, fs⟩
   */
  | {focus: Exp(Lift(a)), frame, stack} => Some({focus: AExp(a), frame, stack})
