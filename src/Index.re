...
let id = x => FFS4Delta.Lam({vid: x, exp: Lift(Var(x))});

ReactDOMRe.render(
  <VizTrace program={FFS4Delta.Lift(App(id("x"), id("y")))} />,
  makeContainer("FFS"),
);

/* TODO: moves numbers around for some reason */
ReactDOMRe.render(
  <VizTrace program={FFS4Delta.Lift(Add(Num(1), Add(Num(2), Num(3))))} />,
  makeContainer("1 + (2 + 3)"),
);

ReactDOMRe.render(
  <VizTrace
    program={FFS4Delta.Let("x", Num(5), Let("y", Num(6), Lift(Add(Var("x"), Var("y")))))}
  />,
